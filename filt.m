...
function [smoothdata,filtwts] = filt(data,srate,locutoff,hicutoff,epochframes,filtorder, revfilt)
...
while filtorder*3 > epochframes, 
    filtorder=filtorder-1;
    % Matlab filtfilt() restriction
    fprintf('Warning:eegfilt(): filter order is %d. The data length may be too short  ',filtorder);
end
